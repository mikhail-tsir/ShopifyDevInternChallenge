...
case class UserRequest[A] @Inject() (user: User, request: Request[A])(implicit messagesApi: MessagesApi) extends MessagesRequest[A](request, messagesApi)

class AuthenticatedUserAction @Inject() (val parser: BodyParsers.Default)(implicit
  val executionContext: ExecutionContext,
  userDao: UserDAO,
  messagesApi: MessagesApi) extends ActionBuilder[UserRequest, AnyContent] {
